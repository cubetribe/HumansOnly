...
export default function Retweet({ tweetId, tweetAuthor }: TweetOptionsProps) {
    ...
    const mutation = useMutation({
        mutationFn: (variables: { isRetweeted: boolean }) => updateReposts(tweetId, tweetAuthor, variables.isRetweeted),
        onMutate: () => {
    ...
    const handleRetweet = () => {
        ...
        const isRetweetedBy = retweetedBy?.some((user: { id: string }) => user.id === tokenOwnerId);
